...
if (random _wave >= 1) then {
	...
	for [{_i=1}, {_i <=_div}, {_i=_i+1}] do {
		...
		_grp = ((getMarkerPos (BONYO_var_enemySpawn_vic call BIS_fnc_selectRandom)) call BONYO_fnc_spawnVicGroup);
...
if (random _wave >= 1) then {
	...
	for [{_i=1}, {_i <=_div}, {_i=_i+1}] do {
		...
		_grp = ((getMarkerPos (BONYO_var_enemySpawn_air call BIS_fnc_selectRandom)) call BONYO_fnc_spawnAirGroup);
